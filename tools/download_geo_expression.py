"""
Download GEO datasets and generate ranked lists.
...
try:
    import GEOparse  # pyright: ignore[reportMissingImports]
except ImportError:
...
# Maps each suffixed GEO ID to keywords. At least one must appear in the
# combined sample metadata text, case-insensitively. This is simpler than key-value
...
# Explicit title patterns for datasets with no characteristics.
# Maps each GEO ID to (case_substring, ctrl_substring). Both are matched
# case-insensitively against the sample title after other strategies fail.
TITLE_PATTERN_RULES = {
...
def base_geo_id(geo_id):
    """Strip a GEO ID to its GSE prefix and digits."""
...
def build_probe_to_entrez(gpl_table):
    ...
    id_col = next(
        (
            ...
            if c.strip().upper() in ("ID", "PROBE_ID", "PROBEID")
        ),
        None,
    )
...
def _kw_score(text):
    """Return positive scores for case evidence and negative scores for controls."""
    score = 0
    for w in CASE_WORDS:
        if w in text:
            score += 1
...
def classify_samples_by_study(gsms, geo_id=None):
    """
    Classify case and control samples within one study.

    The function tries three strategies in order:

    1. Characteristic key polarization:
      A. Key has explicit case values (score>0) AND control values (score<0).
      B. Key has explicit control values; other present values become cases.
         Handles "Diagnosis: AML" present only in disease samples, absent in normals.
      C. Key has explicit case values; other values become controls.

    2. Source and title scoring:
      Scores source_name and title. Characteristics are excluded because a
      study-level disease name may appear in every sample.

    3. Explicit title patterns from TITLE_PATTERN_RULES:
    ...
    for key in all_keys:
        ...
        val_score = {
            v: (_kw_score(v) if v != "_absent_" else 0) for v in val_to_samples
        }
        ...
        if pos_vals and neg_vals:
            ...
            polarity = max(val_score[v] for v in pos_vals) - min(
                val_score[v] for v in neg_vals
            )
            assignment = {}
            ...
            # B: use other present values as cases.
            case_candidates = [
                v for v in val_to_samples if v not in neg_vals and v != "_absent_"
            ]
            if case_candidates:
            ...
            # C: use other values as controls.
        ...
        if assignment is not None and polarity > best_polarity:
            has_case = any(label == 1 for label in assignment.values())
            has_ctrl = any(label == 0 for label in assignment.values())
    ...
    # Use samples with zero case evidence as controls when no control evidence
    # is present.
...
def apply_subset_rule(gsms, geo_id):
    ...
    if not filtered:
        log.warning(
            f"  Subset rule for {geo_id} matched 0/{len(gsms)} samples; using all"
        )
        return gsms
...
def build_expression_matrix(gse, sample_ids):
    """
    Return a genes-by-samples DataFrame for sample_ids.
    ...
    for col in VALUE_COLS:
        try:
            ...
            log.debug("Could not extract expression column %s: %s", col, exc)
            continue
...
def ranked_list_from_matrix(expr, condition):
    """
    expr:      genes-by-samples DataFrame
    ...
    if n_case < 2 or n_ctrl < 2:
        raise ValueError(
            f"Need at least 2 samples per group for Welch t-test; "
    ...
    s = pd.Series(
        np.asarray(t_stats, dtype=np.float64),
...
def aggregate_probes_to_genes(probe_series, probe2entrez):
    """
    Map probe IDs to Entrez IDs. When multiple probes map to the same gene,
...
def process_geo(geo_id, cache_dir, out_dir, overwrite=False):
    ...
    if not probe2entrez:
        log.error(f"  Could not build probe-to-Entrez map for {gse_id}")
        return False
    log.info(f"  Platform {gpl.name}: {len(probe2entrez)} probe-to-Entrez mappings")

    # Sample subset
    gsms = apply_subset_rule(dict(gse.gsms), geo_id)

    # Classify case and control samples from within-study variation.
    assignment = classify_samples_by_study(gsms, geo_id=geo_id)
    if assignment is None:
        log.error(
            f"  Could not classify case/control for {geo_id}. "
            "No characteristic key separates disease and normal samples. "
            f"Run with --geos {geo_id} --debug-chars to inspect metadata."
        )
        return False
    ...
    log.info(f"  Expression matrix: {expr.shape[0]} probes x {expr.shape[1]} samples")

    # T-statistics per probe
    probe_t = ranked_list_from_matrix(expr, condition)

    # Aggregate probes to Entrez genes.
...
def main():
    ...
    p.add_argument(
        "--geo2kegg",
        default="paper/data/geo2kegg.txt",
        help="GEO-to-disease mapping (default: paper/data/geo2kegg.txt)",
    )
    p.add_argument(
        "--out-dir",
    ...
    geo2kegg_path = (
        ROOT / args.geo2kegg
        if not Path(args.geo2kegg).is_absolute()
        else Path(args.geo2kegg)
    )
    out_dir = (
        ROOT / args.out_dir
    ...
    log.info(f"Processing {len(geo_ids)} GEO datasets in {out_dir}")
    ...
    if failed:
        ...
        print(
            "  uv run python tools/download_geo_expression.py --geos <ID> --overwrite"
        )
