...
def t_score(x_w, x_b1, x_b2):
    ...
    v_b = (np.sum((x_b1 - mean_b) ** 2) + np.sum((x_b2 - mean_b) ** 2)) / n_b
...
def andes(
    ...
    rand_indexes1 = [random.sample(g1_population, len(indexes1)) for _ in range(ite)]
    rand_indexes2 = [random.sample(g2_population, len(indexes2)) for _ in range(ite)]
...
def t_score_with_background_correction(
    terms,
    matrix,
    g1_term2index,
    g2_term2index,
    ...
    rand_indexes1 = [
        random.sample(g1_population, len(g1_true_index)) for _ in range(ite)
    ]
    rand_indexes2 = [
        random.sample(g2_population, len(g2_true_index)) for _ in range(ite)
    ]

    random_scores = np.empty(ite)

    for k in range(ite):
        random_scores[k] = t_score(
            matrix[np.ix_(rand_indexes1[k], rand_indexes2[k])],
...
def gsea_andes(term, ranked_list, matrix, term2indices, annotated_indices, ite=1000):
    ...
    rand_indexes = [
        random.sample(annotated_indices, len(term_indices)) for _ in range(ite)
    ]
